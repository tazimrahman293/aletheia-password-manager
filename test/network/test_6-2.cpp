...
int insertTestAccount(Storage &storage, Authenticator &auth, int userID, int suffix)
{
    Account account;
    ...
    account.hash = hashToChars(auth.Encrypt("password"));
    account.expiry = 0;
    account.created = 0;
...
User parseUserFromBody(const std::string &body)
{
    using json = nlohmann::json;
    auto data = json::parse(body);
    User user;
    user.pk = data.at("pk").get<int>();
...
Account parseAccountFromBody(const std::string &body)
{
    using json = nlohmann::json;
    auto data = json::parse(body);
    Account account;
    account.pk = data.at("pk").get<int>();
    ...
    account.created = data.at("created").get<long>();
    account.lastAccessed = data.at("lastAccessed").get<long>();
    account.lastModified = data.at("lastModified").get<long>();
    account.expiry = data.at("expiry").get<long>();
    account.userID = data.at("userID").get<int>();
    return account;
}
...
TEST_CASE("api-create-user") {
    ...
    SUBCASE("valid") {
        ...
        auto user = parseUserFromBody(response->body);
...
TEST_CASE("api-update-user") {
    ...
    SUBCASE("valid") {
        ...
        auto patchedUser = parseUserFromBody(response->body);
...
TEST_CASE("api-get-account") {
    ...
    SUBCASE("valid-user") {
        ...
        for (int i = 0; i < numAccounts; i++) {
            insertTestAccount(storage, auth, userID, i);
        }
...
TEST_CASE("api-update-account") {
    ...
    int userID = insertTestUser(storage, auth, 0);
    int accountID = insertTestAccount(storage, auth, userID, 0);
    ...
    SUBCASE("valid") {
        ...
        auto patchedAccount = parseAccountFromBody(response->body);
...
TEST_CASE("api-login") {
    ...
    SUBCASE("success") {
        ...
        auto loggedInUser = parseUserFromBody(response->body);
